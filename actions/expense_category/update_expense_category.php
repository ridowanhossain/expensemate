...
$currentStmt = $conn->prepare("SELECT name, comment FROM expense_categories WHERE id = ?");
...
$stmt = $conn->prepare("UPDATE expense_categories SET name = ?, comment = ? WHERE id = ?");
$stmt->bind_param('ssi', $name, $comment, $id);

if ($stmt->execute()) {
    echo json_encode(['success' => true, 'message' => 'expense category updated successfully.']);
} else {
    echo json_encode(['success' => false, 'message' => 'Failed to update expense category: ' . $stmt->error]);
}
