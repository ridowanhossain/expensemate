$(document).ready(function () {
  ...
  const table = $("#datatablesSimple").DataTable({
    ajax: {
      url: "actions/fetch_income_statement.php",
      type: "POST",
    ...
    buttons: [
      ...
      },
      {
        extend: "print",
        text: "Print",
        title: "", // We'll handle the title insertion manually inside the customize function
        customize: function (win) {
          // Get the selected date range from the date picker
          const dateRange = $("#dateRangeFilter").val() || "All Dates";

          // Insert the title inside <h4> and center it
          $(win.document.body).prepend(`
                        <h4 style="text-align: center; margin-bottom: 20px;">Income Statement: (${dateRange})</h4>
                    `);
  ...
  $("#dateRangeFilter").daterangepicker(
    ...
    function (start, end) {
      ...
      $.fn.dataTable.ext.search.push(function (settings, data) {
        const tableDate = moment(data[4], "DD/MM/YYYY"); // Date column should match this format
